...
public class SpawnEntityEffect extends AbstractEffect implements Listener {
    ...
    private void configureLivingEntity(LivingEntity living, Player owner, LivingEntity target,
        ...
        if (speedAttr != null) {
            ...
            debug("WARNING: Entity " + living.getType() + " has no GENERIC_MOVEMENT_SPEED attribute!");
        }
        ...
        if (damage != null) {
            ...
            debug("WARNING: Entity " + living.getType() + " has no GENERIC_ATTACK_DAMAGE attribute!");
        }

        // Knockback Resistance
        AttributeInstance kbResist = living.getAttribute(Attribute.GENERIC_KNOCKBACK_RESISTANCE);
        if (kbResist != null) {
        ...
        // Baby
        if (params.getBoolean("baby", false) && living instanceof Ageable ageable) {
            ageable.setBaby();
        }
        ...
        Bukkit.getScheduler().runTaskLater(getPlugin(), () -> {
            if (living.isValid() && !living.isDead()) {
                ...
                AttributeInstance finalSpeed = living.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED);
                if (finalSpeed != null) {
                ...
                AttributeInstance finalDamage = living.getAttribute(Attribute.GENERIC_ATTACK_DAMAGE);
                if (finalDamage != null) {
        ...
        Bukkit.getScheduler().runTaskLater(getPlugin(), () -> {
            if (living.isValid() && !living.isDead()) {
                ...
                AttributeInstance spd = living.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED);
                AttributeInstance dmg = living.getAttribute(Attribute.GENERIC_ATTACK_DAMAGE);
    ...
    public void onSpawnedEntityAttack(EntityDamageByEntityEvent event) {
        ...
        AttributeInstance damageAttr = attacker.getAttribute(Attribute.GENERIC_ATTACK_DAMAGE);
        if (damageAttr != null) {
