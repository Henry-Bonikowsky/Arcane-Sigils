...
public class SpawnDisplayEffect extends AbstractEffect {
    ...
    public boolean execute(EffectContext context) {
        ...
        for (Location loc : spawnLocations) {
            // For OUTWARD mode, calculate yaw to face away from center
            if (outwardFacing) {
                double dx = loc.getX() - baseLoc.getX();
                double dz = loc.getZ() - baseLoc.getZ();
                // Calculate yaw to face outward (away from center)
                // atan2(-dx, dz) gives angle facing outward
                float yaw = (float) Math.toDegrees(Math.atan2(-dx, dz));
                loc.setYaw(yaw);
            }

            Display display = spawnDisplay(loc, displayType, value, scaleX, scaleY, scaleZ, glow, billboard, params, outwardFacing);
            if (display != null) {
    ...
    private void startAnimatedRotation(List<Display> displays, Location center, Player owner,
        ...
        // Use the INTENDED center (player location), not where displays spawned
        Location trueCenter = owner.getLocation();

        // Store each display's base offset from TRUE center (for rotation calculation)
        // For snap_to_ground, we only store X/Z offsets since Y will be recalculated
        List<double[]> baseOffsets = new ArrayList<>();
        for (Display display : displays) {
            Location displayLoc = display.getLocation();
            double dx = displayLoc.getX() - trueCenter.getX();
            double dy = displayLoc.getY() - trueCenter.getY();
            double dz = displayLoc.getZ() - trueCenter.getZ();
            ...
            public void run() {
                ...
                for (int i = 0; i < displays.size(); i++) {
                    ...
                    if (snapToGround && player != null) {
                        ...
                        newLoc = new Location(player.getWorld(), targetX, groundY, targetZ);
                    } else {
    ...
    private Display spawnDisplay(Location loc, DisplayType type, String value, float scaleX, float scaleY, float scaleZ,
                                  boolean glow, Display.Billboard billboard, EffectParams params, boolean outwardFacing) {
        if (loc.getWorld() == null) return null;

        Display display;

        // Get rotation from location (used for OUTWARD billboard mode)
        ...
        if (display != null) {
            // Calculate rotation for OUTWARD mode or scale transform
            boolean needsTransform = outwardFacing ||
                                     Math.abs(scaleX - 1.0f) > 0.01f ||
                                     Math.abs(scaleY - 1.0f) > 0.01f ||
                                     Math.abs(scaleZ - 1.0f) > 0.01f;

            if (needsTransform) {
                // Convert yaw to radians for Y-axis rotation (outward facing)
                float yawRadians = outwardFacing ? (float) Math.toRadians(yaw) : 0;

                Transformation transform = new Transformation(
                    new Vector3f(0, 0, 0), // Translation
                    new AxisAngle4f(yawRadians, 0, 1, 0), // Left rotation (Y-axis rotation for yaw)
                    new Vector3f(scaleX, scaleY, scaleZ), // Scale (non-uniform)
    ...
    private void scheduleFollowing(List<Display> displays, Player owner, Location spawnBase, int duration, boolean snapToGround, double groundOffset) {
        ...
        // Start a repeating task to move displays
        BukkitRunnable followTask = new BukkitRunnable() {
            ...
            public void run() {
                ...
                // Move each display to maintain its offset from player
                for (Display display : displays) {
                    if (!display.isValid()) continue;
                    ...
                    if (snapToGround) {
                        ...
                        double targetX = playerLoc.getX() + offset.getX();
                        double targetZ = playerLoc.getZ() + offset.getZ();
                        double groundY = findGroundBelowPlayer(playerLoc.getWorld(), targetX, playerLoc.getY(), targetZ);
                        groundY += groundOffset; // Apply ground offset
                        newLoc = new Location(playerLoc.getWorld(), targetX, groundY, targetZ);
                    } else {
                        // Normal following: maintain fixed offset from player
                        newLoc = playerLoc.clone().add(offset);
                    }
                    display.teleport(newLoc);
                }
            }
        };
        followTask.runTaskTimer(getPlugin(), 2L, 2L); // Move every 2 ticks (0.1s)
