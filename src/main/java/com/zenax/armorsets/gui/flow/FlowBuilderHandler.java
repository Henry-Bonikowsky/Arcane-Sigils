...
public class FlowBuilderHandler extends AbstractHandler {
    ...
    private void autoSave(Player player, GUISession session) {
        ...
        com.zenax.armorsets.utils.LogHelper.debug("[FlowBuilder] Saving flow for sigil '%s', type=%s, trigger=%s",
            sigil.getId(), flowConfig.getType(), flowConfig.getTrigger());
        com.zenax.armorsets.utils.LogHelper.debug("[FlowBuilder] Flow has %d nodes, graph=%s",
        ...
        if (flowConfig.isAbility()) {
            ...
            if (existingAbility != null) {
                ...
                com.zenax.armorsets.utils.LogHelper.debug("[FlowBuilder] Removed existing ABILITY flow: %s", removed);
            }
        } else {
            boolean removed = sigil.removeFlowByTrigger(flowConfig.getTrigger());
            com.zenax.armorsets.utils.LogHelper.debug("[FlowBuilder] Removed existing SIGNAL flow for trigger '%s': %s",
                flowConfig.getTrigger(), removed);
        }

        boolean added = sigil.addFlow(flowConfig);
        com.zenax.armorsets.utils.LogHelper.debug("[FlowBuilder] Added flow to sigil: %s, sigil now has %d flows",
        ...
        com.zenax.armorsets.utils.LogHelper.debug("[FlowBuilder] Sigil hasFlows=%s before save", sigil.hasFlows());
    ...
    private void handleTest(Player player, GUISession session) {
        ...
        // Create test context
        com.zenax.armorsets.effects.EffectContext effectContext =
                com.zenax.armorsets.effects.EffectContext.builder(player, com.zenax.armorsets.events.SignalType.INTERACT)
    ...
    private void handleCanvasClick(Player player, GUISession session, int canvasX, int canvasY, InventoryClickEvent event) {
        ...
        if (nodeAtPos != null) {
            ...
            if (nodeAtPos.getId().equals(selectedId)) {
                // Already selected - open param editor (END has no params, START does)
                if (nodeAtPos.getType() != NodeType.END) {
                    playSound(player, "click");
                    NodeConfigHandler.openGUI(guiManager, player, session, nodeAtPos, 0);
                } else {
                    playSound(player, "error");
                    player.sendMessage(TextUtil.colorize("§7" + nodeAtPos.getDisplayName() + " has no configurable parameters."));
                }
            } else {
                // Select node
                session.put("selectedNode", nodeAtPos.getId());
    ...
    private void refreshGUI(Player player, GUISession session) {
        ...
        String signalKey = session.get("signalKey", String.class);
        FlowGraph graph = session.get("flow", FlowGraph.class);
        openGUI(guiManager, player, sigil, signalKey, graph, session);
    }
