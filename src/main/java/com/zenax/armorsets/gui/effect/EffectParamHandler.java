...
public class EffectParamHandler extends AbstractHandler {
    ...
    private void handleSave(Player player, GUISession session, Sigil sigil, String signalKey) {
        ...
        // Get effects list based on mode (ability vs signal)
        List<String> effects;
        boolean isAbilityMode = "ability".equals(signalKey);
        ...
        if (isAbilityMode) {
            ...
            effects = activation.getEffects();
        } else {
            // Signal effects are now handled by FlowBuilder nodes, not effect lists
            player.sendMessage(TextUtil.colorize("§cEffect editing moved to Flow Builder!"));
            return;
        }

        // Add or update effect
        if (effectIndex != null && effectIndex >= 0 && effectIndex < effects.size()) {
            // Update existing effect
            effects.set(effectIndex, effectString);
            player.sendMessage(TextUtil.colorize("§aEffect updated!"));
        } else {
            // Add new effect
            effects.add(effectString);
            player.sendMessage(TextUtil.colorize("§aEffect added!"));
        }

        // Show tier param creation message
        if (scaledParams != null && !scaledParams.isEmpty()) {
            for (String paramKey : scaledParams.keySet()) {
                if (scaledParams.get(paramKey)) {
                    player.sendMessage(TextUtil.colorize("§7Tier parameter §e{" + paramKey + "} §7created"));
                }
            }
        }
        ...
        // Return to Flow Builder (replaces old effect list)
        if (isAbilityMode) {
            com.zenax.armorsets.gui.sigil.SigilEditorHandler.openGUI(guiManager, player, sigil);
        } else {
            com.zenax.armorsets.gui.flow.FlowBuilderHandler.openGUI(guiManager, player, sigil, signalKey);
