...
public class ConditionManager {
    ...
    public boolean checkConditions(List<String> conditions, EffectContext context,
        ...
        com.zenax.armorsets.utils.LogHelper.debug("[Conditions] Checking %d conditions with %s logic", conditions.size(), logic);
        ...
        if (logic == ConditionLogic.OR) {
            ...
            for (String condition : conditions) {
                ...
                com.zenax.armorsets.utils.LogHelper.debug("[Conditions] %s = %s (OR mode)", condition, result);
                if (result) {
                    return true; // Any passing condition allows execution
                }
            }
            com.zenax.armorsets.utils.LogHelper.debug("[Conditions] All conditions failed in OR mode");
            return false; // No conditions passed
        } else {
            // AND logic (default): All conditions must pass
            for (String condition : conditions) {
                boolean result = evaluateCondition(condition, context);
                com.zenax.armorsets.utils.LogHelper.debug("[Conditions] %s = %s (AND mode)", condition, result);
                if (!result) {
                    com.zenax.armorsets.utils.LogHelper.debug("[Conditions] Failed condition: %s", condition);
                    return false; // Any failed condition blocks execution
                }
            }
            com.zenax.armorsets.utils.LogHelper.debug("[Conditions] All conditions passed");
        ...
        try {
            return switch (type) {
                ...
                case "ON_GROUND" -> {
                    ...
                    com.zenax.armorsets.utils.LogHelper.debug("[Conditions] ON_GROUND check: player %s isOnGround=%s",
    ...
    private boolean checkHasMark(EffectContext context, String[] parts) {
        ...
        com.zenax.armorsets.effects.MarkManager markManager =
            com.zenax.armorsets.ArmorSetsPlugin.getInstance().getMarkManager();
        ...
        if (parts.length >= 3) {
            ...
            if (targetMod.equals("@self")) {
                ...
                com.zenax.armorsets.binds.TargetGlowManager glowManager =
                    com.zenax.armorsets.ArmorSetsPlugin.getInstance().getTargetGlowManager();
                if (glowManager != null) {
    ...
    private boolean checkHoldingSigilItem(EffectContext context) {
        ...
        if (sourceItem == null) {
            com.zenax.armorsets.utils.LogHelper.debug("[Conditions] HOLDING_SIGIL_ITEM: No sourceItem in context");
            return false;
        }
        ...
        if (mainHand != null && sourceItem.isSimilar(mainHand)) {
            com.zenax.armorsets.utils.LogHelper.debug("[Conditions] HOLDING_SIGIL_ITEM: Holding in main hand");
            return true;
        }
        ...
        if (offHand != null && sourceItem.isSimilar(offHand)) {
            com.zenax.armorsets.utils.LogHelper.debug("[Conditions] HOLDING_SIGIL_ITEM: Holding in off hand");
            return true;
        }

        com.zenax.armorsets.utils.LogHelper.debug("[Conditions] HOLDING_SIGIL_ITEM: Not holding sigil item");
        return false;
    }
    ...
    private boolean checkDurabilityPercent(EffectContext context, String[] parts) {
        ...
        org.bukkit.inventory.ItemStack sourceItem = context.getMetadata("sourceItem", null);
        if (sourceItem == null) {
            com.zenax.armorsets.utils.LogHelper.debug("[Conditions] DURABILITY_PERCENT: No sourceItem in context");
            return false;
        }

        // Check if item has durability
        if (!(sourceItem.getItemMeta() instanceof org.bukkit.inventory.meta.Damageable damageable)) {
            com.zenax.armorsets.utils.LogHelper.debug("[Conditions] DURABILITY_PERCENT: Item is not damageable");
