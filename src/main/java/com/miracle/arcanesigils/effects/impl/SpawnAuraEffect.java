...
public class SpawnAuraEffect extends AbstractEffect {
    ...
    public boolean execute(EffectContext context) {
        ...
        // Also check params map directly in case fromMap didn't set the field
        if (amplifier == 0 && params.getInt("amplifier", 0) > 0) {
            amplifier = params.getInt("amplifier", 0);
        }
        boolean showParticles = params.getBoolean("show_particles", true);
        String affectsStr = params.getString("affects", "ENEMIES");

        debug("SPAWN_AURA params: amplifier=" + amplifier + ", getInt('amplifier')=" + params.getInt("amplifier", -1) +
              ", effect_type=" + effectTypeStr + ", radius=" + radius + ", full params=" + params);
