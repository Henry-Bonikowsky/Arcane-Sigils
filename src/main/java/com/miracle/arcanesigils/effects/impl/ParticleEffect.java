...
public class ParticleEffect extends AbstractEffect {
    ...
    private void parseKeyValue(EffectParams params, String key, String value) {
        switch (key) {
            ...
            case "ground_offset" -> params.set("ground_offset", parseDouble(value, 0.0));
    ...
    private boolean executePulse(EffectContext context) {
        ...
        // Debug logging for pulse configuration
        com.miracle.arcanesigils.utils.LogHelper.debug(
            "[Pulse] waves=%d, startRadius=%.2f, endRadius=%.2f, waveDelay=%d, pulseShape=%s",
            waves, startRadius, endRadius, waveDelay, pulseShape);
    ...
    private boolean executeShapeParticleMode(EffectContext context, String shapeId, Location center, EffectParams params) {
        // Animation params
        double duration = params.getDouble("duration", 2.0);
        debug("Shape particle mode: duration=" + duration + ", all params=" + params);
        double rotateSpeed = params.getDouble("rotate_speed", 1.0);
        boolean followTarget = params.getBoolean("follow_target", false);
        double yOffset = params.getDouble("y_offset", 0.0);
        ...
        new org.bukkit.scheduler.BukkitRunnable() {
            ...
            public void run() {
                ...
                if (tick % tickRate == 0) {
                    ...
                    Location currentCenter;
                    if (finalTarget != null && finalTarget.isValid()) {
                        currentCenter = finalTarget.getLocation().add(0, yOffset, 0);
                    } else {
                    ...
                    // Apply snap_to_ground if enabled
                    if (snapToGround && currentCenter.getWorld() != null) {
                        double baseY = finalTarget != null ? finalTarget.getLocation().getY() : currentCenter.getY();
                        for (Location point : shapePoints) {
                            double groundY = findGroundY(point.getWorld(), point.getX(), baseY, point.getZ());
                            point.setY(groundY + groundOffset);
                        }
                    }
     ...
     * Searches downward from baseY to find the first solid block.
     */
    private double findGroundY(org.bukkit.World world, double x, double baseY, double z) {
        ...
        for (int y = startY; y >= world.getMinHeight(); y--) {
            if (world.getBlockAt(blockX, y, blockZ).getType().isSolid()) {
                return y + 1.0; // Top of the solid block
            }
        }
        return baseY; // Fallback to original Y
    }
