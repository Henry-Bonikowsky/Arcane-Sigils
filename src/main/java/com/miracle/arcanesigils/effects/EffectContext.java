...
public class EffectContext {
    ...
    public void cancelEvent() {
        if (bukkitEvent instanceof Cancellable cancellable) {
            ...
            this.cancelled = true;
            // DEBUG: Log who is cancelling the event
            org.bukkit.Bukkit.getLogger().warning("[DEBUG] cancelEvent() called for " + player.getName() +
                " signal=" + signalType + " sigil=" + sigilId);
            // Print abbreviated stack trace to identify caller
            StackTraceElement[] stack = Thread.currentThread().getStackTrace();
            for (int i = 2; i < Math.min(6, stack.length); i++) {
                org.bukkit.Bukkit.getLogger().warning("  at " + stack[i]);
            }
        }
    }
