        ...
        for (Entity entity : nearbyEntities) {
            ...
            if (aura.effectType != null) {
                living.addPotionEffect(new PotionEffect(
                    aura.effectType,
                    60, // 3 seconds - longer than tick interval to prevent flickering
                    aura.amplifier,
                    false, // ambient
    ...
    public void onOwnerHit(UUID ownerUUID) {
        ...
        for (ActiveAura aura : activeAuras.values()) {
            ...
            for (UUID enemyUUID : aura.affectedEnemies) {
                ...
                if (distance > 0.3) { // Don't pull if already very close
                    // Normalize first to get direction
                    direction.normalize();

                    // Strong consistent pull - pullStrength 1.0 = solid grapple
                    // Higher strength = faster pull, capped at reasonable speed
                    double basePull = Math.min(1.5, 0.8 + (distance * 0.15)); // Stronger when far
                    double pullMagnitude = basePull * aura.pullStrength;

                    // Apply horizontal pull
                    direction.multiply(pullMagnitude);

                    // Add slight upward lift to help with terrain
                    direction.setY(Math.max(direction.getY(), 0.15));

                    living.setVelocity(direction);
                    // Particles removed - no visual feedback for pull
                }
            }
