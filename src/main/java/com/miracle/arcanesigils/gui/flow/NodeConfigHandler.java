...
import com.miracle.arcanesigils.gui.effect.EffectParamHandler;
import com.miracle.arcanesigils.gui.effect.EffectParamHandler.EffectParamConfig;
import com.miracle.arcanesigils.gui.effect.EffectParamHandler.ParamInfo;
import com.miracle.arcanesigils.gui.effect.EffectParamHandler.ParamType;
import com.miracle.arcanesigils.gui.effect.ParticleSelectorHandler;
import com.miracle.arcanesigils.gui.effect.SoundSelectorHandler;
...
public class NodeConfigHandler extends AbstractHandler {
    ...
    private void openAttributeBrowser(Player player, GUISession session, EffectNode node, String paramKey) {
        ...
        com.miracle.arcanesigils.gui.effect.AttributeSelectorHandler.openGUI(guiManager, player,
            selectedAttribute -> {
                ...
                player.sendMessage(TextUtil.colorize("§aAttribute set to: §f" +
                    com.miracle.arcanesigils.effects.impl.ModifyAttributeEffect.getAttributeDisplayName(selectedAttribute)));
    ...
    private void handleConditionParam(Player player, GUISession session, ConditionNode node, int slot) {
        if (slot == 0) {
            ...
            playSound(player, "click");
            com.miracle.arcanesigils.gui.condition.FlowConditionSelectorHandler.openGUI(guiManager, player, conditionSession);
        }
    }
     ...
     * Uses the same unified tier system as Effect nodes.
     */
    private void toggleStartParamTierScaling(Player player, GUISession session, FlowNode node, String paramKey, String displayName, double defaultValue) {
        ...
        // Check if currently using any tier placeholder
        String currentPlaceholder = findCurrentPlaceholder(node, paramKey);
        boolean isCurrentlyScaled = currentPlaceholder != null;

        if (isCurrentlyScaled) {
            // Turn off tier scaling - set back to the tier 1 value from TierScalingConfig
            com.miracle.arcanesigils.tier.TierScalingConfig tierConfig = sigil.getTierScalingConfig();
            double baseValue = defaultValue;
            if (tierConfig != null && tierConfig.hasParam(currentPlaceholder)) {
                baseValue = tierConfig.getParamValue(currentPlaceholder, 1);
            }
            node.setParam(paramKey, baseValue);
            player.sendMessage(TextUtil.colorize("§eTier scaling disabled for §f" + displayName));
            ...
            // Start node params (chance, cooldown) are already descriptive - use as-is
            String placeholderName = paramKey.toLowerCase();

            // Ensure sigil has TierScalingConfig
            com.miracle.arcanesigils.tier.TierScalingConfig tierConfig = sigil.getTierScalingConfig();
            if (tierConfig == null) {
                tierConfig = new com.miracle.arcanesigils.tier.TierScalingConfig();
                sigil.setTierScalingConfig(tierConfig);
            }

            // Register param with default scaling if not already present
            if (!tierConfig.hasParam(placeholderName)) {
                tierConfig.addScaledParam(placeholderName, baseValue, maxTier);
            }

            // Set node param to placeholder
            node.setParam(paramKey, "{" + placeholderName + "}");
            ...
            player.sendMessage(TextUtil.colorize("§7Placeholder: §e{" + placeholderName + "}"));
    ...
    // Generic param names that need prefixing to avoid conflicts
    private static final java.util.Set<String> GENERIC_PARAM_NAMES = java.util.Set.of(
        "value", "amount", "count", "number", "level", "strength", "power"
    );

    /**
     * Generate a unique tier scaling placeholder name for a parameter.
     * Generic names like "value" get prefixed with the effect type to avoid conflicts.
     */
    private String generateUniquePlaceholderName(FlowNode node, String paramKey) {
        // If it's already a descriptive name, use it as-is
        if (!GENERIC_PARAM_NAMES.contains(paramKey.toLowerCase())) {
            return paramKey.toLowerCase();
        }

        // For Effect nodes, prefix with effect type
        if (node instanceof EffectNode effectNode && effectNode.getEffectType() != null) {
            String effectType = effectNode.getEffectType().toLowerCase();
            // Clean up effect type for use in placeholder (remove underscores, use short form)
            effectType = effectType.replace("_", "");
            return effectType + "_" + paramKey.toLowerCase();
        }

        // For other nodes, prefix with node type
        return node.getType().name().toLowerCase() + "_" + paramKey.toLowerCase();
    }

    /**
     * Check if a node param is currently using any tier scaling placeholder.
     * Returns the placeholder name if found, null otherwise.
     */
    private String findCurrentPlaceholder(FlowNode node, String paramKey) {
    ...
    private void toggleTierScaling(Player player, GUISession session, FlowNode node, ParamInfo paramInfo) {
        ...
        if (isCurrentlyScaled) {
            ...
            String placeholderName = generateUniquePlaceholderName(node, paramKey);
            ...
            // Set node param to placeholder
            node.setParam(paramKey, "{" + placeholderName + "}");
            session.put("currentScaledParam", placeholderName);
    ...
    /**
     * Check if a param value is a tier placeholder (e.g., "{damage}" or "{dealdamage_value}").
     * Now detects any placeholder pattern, not just exact param key match.
     */
    private static boolean isTierPlaceholder(Object value, String paramKey) {
        if (value == null) return false;
        String strVal = value.toString();
        // Check if it's any placeholder format
        return strVal.startsWith("{") && strVal.endsWith("}") && strVal.length() > 2;
    }
