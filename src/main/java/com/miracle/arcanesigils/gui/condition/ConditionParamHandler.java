...
public class ConditionParamHandler extends AbstractHandler {
    ...
    private void handleValueInput(Player player, GUISession session, Sigil sigil, String signalKey,
        ...
        try {
            ...
            player.sendMessage(com.miracle.arcanesigils.utils.TextUtil.colorize("§cFailed to open sign input. Please try again."));
            e.printStackTrace();
        }
    }
    ...
    private void handleSave(Player player, GUISession session, Sigil sigil, String signalKey,
        ...
        // Get conditions list from FlowConfig
        com.miracle.arcanesigils.flow.FlowConfig flowConfig = sigil.getFlowForTrigger(signalKey);
        List<String> conditions = flowConfig != null ? flowConfig.getConditions() : new ArrayList<>();
