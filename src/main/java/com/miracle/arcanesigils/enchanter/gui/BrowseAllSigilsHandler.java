...
/**
 * Browse All Sigils - paginated list of non-exclusive sigils.
 * Layout: 3 rows (27 slots)
 */
public class BrowseAllSigilsHandler extends AbstractHandler {
    ...
    public BrowseAllSigilsHandler(ArmorSetsPlugin plugin, GUIManager guiManager) {
    ...
    public void handleClick(Player player, GUISession session, int slot, InventoryClickEvent event) {
        ...
        if (slot == NEXT_PAGE_SLOT) {
            int page = session.getInt("page", 1);
            List<Sigil> sigils = getNonExclusiveSigils();
        ...
        if (slot < 18) {
            ...
            if (clickedItem != null && clickedItem.getType() != Material.GRAY_STAINED_GLASS_PANE) {
                ...
                if (sigilId != null) {
                    ...
                    tierSession.put("parentType", GUIType.ENCHANTER_BROWSE_ALL);
    ...
    public void reopen(Player player, GUISession session) {
        ...
        Inventory inv = Bukkit.createInventory(null, 27,
            TextUtil.colorize("§7Enchanter > §fAll Sigils §7(Page " + page + "/" + maxPage + ")"));
    ...
    private List<Sigil> getNonExclusiveSigils() {
